...
import { BUFFER_SEVERITY, STATUS_VARIANT } from '@/src/shared/status/ui'
...
const ForecourtSectionHeading = ({
  ...
  <div
    id={id}
    className="scroll-mt-20 rounded border bg-[var(--surface-muted)] p-4"
  >
...
export type AdminForecourtClientProps = {
  isTanzania: boolean
}

export default function AdminForecourtClient({
  isTanzania,
}: AdminForecourtClientProps) {
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="space-y-3 p-4">
          <div>
            <div className="text-sm font-semibold">Forecourt workspace</div>
            <div className="text-xs text-[var(--text-secondary)]">
              Navigate by technician workflow, or open the relevant setup page
              directly.
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 border-t pt-3">
            ...
            <Button asChild size="sm" variant="ghost">
              <Link href="/settings/pumps">Pump settings</Link>
            </Button>
            <Button asChild size="sm" variant="ghost">
              <Link href="/settings/tanks">Tank settings</Link>
            </Button>
            <Button asChild size="sm" variant="ghost">
              <Link href="/setup/forecourt/pricing">Fuel pricing</Link>
            </Button>
            <Button asChild size="sm" variant="ghost">
              <Link href="/admin/settings">POS / JPL integration</Link>
            </Button>
            <Button asChild size="sm" variant="ghost">
      ...
      <CollapsibleStatusSection
        title="Forecourt connection"
        status={forecourtBadge(forecourtConn?.status).label}
        statusVariant={forecourtBadge(forecourtConn?.status).variant}
      ...
      <ForecourtSectionHeading
        id="commissioning"
        title="Commissioning"
        description="Validate the live connection, complete first-site checks, and gather release evidence before operator handover."
      />

      <DeferredForecourtPanel label="Commissioning readiness">
        <JplCommissioningReadinessPanel isTanzania={isTanzania} />
      </DeferredForecourtPanel>

      <DeferredForecourtPanel label="Field validation">
        <JplFieldValidationPanel isTanzania={isTanzania} />
      </DeferredForecourtPanel>

      <ForecourtSectionHeading
        id="configuration"
        title="Configuration"
        description="Review pump, nozzle, tank, product, price, and DOMS identity mappings without issuing controller installation writes."
      />

      <DeferredForecourtPanel label="Configuration reconciliation">
      ...
      <ForecourtSectionHeading
        ...
        description="Review transaction buffers, replay checkpoints, wetstock deliveries, price schedules, and recoverable production workflows."
      />

      <DeferredForecourtPanel label="Workflow review">
      ...
      <ForecourtSectionHeading
        ...
        description="Prepare, compare, approve, preview, and execute tightly controlled DOMS maintenance actions with explicit safety gates."
      />

      <DeferredForecourtPanel label="Maintenance plan">
        <JplMaintenancePlanPanel />
      </DeferredForecourtPanel>

      <DeferredForecourtPanel label="Maintenance approval gate">
        <JplMaintenanceGatePanel />
      </DeferredForecourtPanel>

      <DeferredForecourtPanel label="Maintenance preview">
        <JplMaintenancePreviewPanel />
      </DeferredForecourtPanel>

      <DeferredForecourtPanel label="Maintenance execution gate">
        <JplMaintenanceExecutionGatePanel />
      </DeferredForecourtPanel>

      <ForecourtSectionHeading
        id="diagnostics"
        title="Diagnostics and support evidence"
        description="Inspect protocol diagnostics, export redacted support evidence, and review live and persisted forecourt events."
      />

      <DeferredForecourtPanel label="JPL diagnostics">
        <JplDiagnosticsPanel />
      </DeferredForecourtPanel>

      <DeferredForecourtPanel label="Support bundle">
      ...
      <CollapsibleStatusSection
        title="Audit log (Postgres: forecourt_events)"
        status={`${filteredRows.length} events`}
        statusVariant={
          filteredRows.length ? STATUS_VARIANT.INFO : STATUS_VARIANT.SUCCESS
        }
