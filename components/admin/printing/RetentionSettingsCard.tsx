...
const groups: Array<{
...
}> = [
  {
    title: 'Printing',
    description:
      'Terminal print jobs only. Pending and processing jobs are never age-pruned.',
    fields: [
  ...
  },
  {
    ...
    description:
      'Successful queue work is kept shorter than failures and resolved dead-letter evidence.',
    fields: [
      { key: 'transactionQueueDoneDays', label: 'Transaction queue success' },
      {
        key: 'transactionQueueFailedDays',
        label: 'Transaction queue terminal failure',
      },
      ...
      {
        key: 'fiscalInboxResolvedDeadDays',
        label: 'Resolved dead fiscal inbox',
      },
    ],
  },
  {
    ...
    description:
      'Retention for logs and bounded forecourt diagnostic evidence.',
    fields: [
      ...
      {
        key: 'forecourtMaintenanceSecurityEventDays',
        label: 'Maintenance/security events',
      },
      {
        key: 'forecourtFieldEvidenceEventDays',
        label: 'Field evidence events',
      },
      { key: 'jplCheckpointClearedDays', label: 'Cleared JPL checkpoints' },
      {
        key: 'jplSupervisedReplayClearedDays',
        label: 'Cleared supervised replay rows',
      },
      {
        key: 'pssParsedCompatibilityDays',
        label: 'PSS parsed compatibility data',
      },
    ],
  },
]
...
export function RetentionSettingsCard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!policyRes.ok)
        throw new Error(
          policyJson?.error ?? 'Failed to load retention settings',
        )
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(json?.error ?? 'Failed to save retention settings')
      setPolicy(json?.data ?? json)
      setMessage({
        variant: 'success',
        text: 'Retention settings saved. Runtime polling will pick them up without a restart.',
      })
    } catch (error: any) {
      setMessage({ variant: 'error', text: error?.message ?? String(error) })
    } finally {
  ...
  const runNow = async () => {
    ...
    try {
      ...
      const deleted =
        Number(result?.retention?.deleted ?? 0) +
        Number(result?.printTestJobs?.deleted ?? 0)
      const examined =
        Number(result?.retention?.examined ?? 0) +
        Number(result?.printTestJobs?.examined ?? 0)
      setMessage({
        variant: 'info',
        text: result?.dryRun
          ? `Dry run completed. ${examined} eligible rows were found; nothing was deleted.`
          : `Cleanup completed. ${deleted} rows were deleted.`,
      })
    } catch (error: any) {
      setMessage({ variant: 'error', text: error?.message ?? String(error) })
    } finally {
  ...
  const setDay = (key: DayField, value: number) =>
  ...
  return (
    <Card>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-secondary)]">
              Production-safe replacement for VPOS_RETENTION_ENABLED,
              VPOS_RETENTION_DRY_RUN and the retention environment values.
              Station settings take precedence over environment defaults.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <Button variant="primary" onClick={save} disabled={!!busy || !csrf}>
              {busy === 'save' ? 'Saving…' : 'Save retention'}
            </Button>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        {message ? (
          <Alert variant={message.variant} title="Retention">
            {message.text}
          </Alert>
        ) : null}
          ...
          <label className="flex items-start gap-3 rounded-2xl border border-[var(--border-default)] bg-[var(--surface-muted)] p-4">
            ...
            <span>
              ...
              <span className="text-xs text-[var(--text-secondary)]">
                Equivalent to VPOS_RETENTION_ENABLED. The worker rechecks this
                station setting every minute.
              </span>
            </span>
          </label>
          <label className="flex items-start gap-3 rounded-2xl border border-[var(--border-default)] bg-[var(--surface-muted)] p-4">
            <Checkbox
              ...
              onChange={() => setPolicy({ ...policy, dryRun: !policy.dryRun })}
            />
            <span>
              <span className="block text-sm font-semibold">Dry-run only</span>
              <span className="text-xs text-[var(--text-secondary)]">
                Equivalent to VPOS_RETENTION_DRY_RUN. Keep enabled while
                validating policy values.
              </span>
            </span>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label className="space-y-1 text-xs font-semibold text-[var(--text-secondary)]">
            ...
            <div className="flex items-center gap-2">
              <Input
                type="number"
                min={1}
                value={String(Math.round(policy.cleanupIntervalMs / 60000))}
                onChange={(event) =>
                  setPolicy({
                    ...policy,
                    cleanupIntervalMs:
                      Math.max(1, Number(event.target.value || 1)) * 60000,
                  })
                }
              />
          ...
          <label className="space-y-1 text-xs font-semibold text-[var(--text-secondary)]">
            <span>Maximum batches per run</span>
            <Input
              type="number"
              min={1}
              max={100}
              value={String(policy.maxBatches)}
              onChange={(event) =>
                setPolicy({
                  ...policy,
                  maxBatches: Number(event.target.value || 1),
                })
              }
            />
          </label>
        ...
        {groups.map((group) => (
          <div
            key={group.title}
            className="space-y-3 rounded-2xl border border-[var(--border-default)] p-4"
          >
            <SectionHeader
              title={group.title}
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              {group.fields.map((field) => (
                <DayInput
                  ...
                  onChange={(value) => setDay(field.key, value)}
                />
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3 rounded-2xl border border-[var(--border-default)] p-4">
          <SectionHeader
            title="Configuration history and payload compaction"
            description="Advanced retention controls. These affect more than printing and should normally remain at their defaults."
          />
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            <label className="space-y-1 text-xs font-semibold text-[var(--text-secondary)]">
              <span>Config versions retained</span>
              <Input
                type="number"
                min={1}
                max={1000}
                value={String(policy.configVersionLimit)}
                onChange={(event) =>
                  setPolicy({
                    ...policy,
                    configVersionLimit: Number(event.target.value || 1),
                  })
                }
              />
            </label>
            <DayInput
              label="Config version minimum age"
              value={policy.configVersionMinAgeDays}
              onChange={(value) => setDay('configVersionMinAgeDays', value)}
            />
            <DayInput
              label="Forecourt payload grace period"
              value={policy.forecourtPayloadGraceDays}
              onChange={(value) => setDay('forecourtPayloadGraceDays', value)}
            />
          </div>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <label className="flex items-start gap-3 rounded-xl bg-[var(--surface-muted)] p-3">
              <Checkbox
                checked={policy.forecourtPayloadCompactionEnabled}
                onChange={() =>
                  setPolicy({
                    ...policy,
                    forecourtPayloadCompactionEnabled:
                      !policy.forecourtPayloadCompactionEnabled,
                  })
                }
              />
              <span className="text-sm">
                Enable forecourt payload compaction
              </span>
            </label>
            <label className="flex items-start gap-3 rounded-xl bg-[var(--surface-muted)] p-3">
              <Checkbox
                checked={policy.forecourtPayloadDryRun}
                onChange={() =>
                  setPolicy({
                    ...policy,
                    forecourtPayloadDryRun: !policy.forecourtPayloadDryRun,
                  })
                }
              />
              <span className="text-sm">
                Forecourt payload compaction dry-run
              </span>
            </label>
          </div>
        </div>
