        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-secondary)]">
              Production-safe replacement for VPOS_RETENTION_ENABLED, VPOS_RETENTION_DRY_RUN and the retention environment values. Station settings take precedence over environment defaults.
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" onClick={runNow} disabled={!!busy || !csrf}>
              {busy === 'run' ? 'Running…' : policy.dryRun ? 'Preview cleanup' : 'Clean up now'}
            </Button>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <label className="flex items-start gap-3 rounded-2xl border border-[var(--border-default)] bg-[var(--surface-muted)] p-4">
            ...
            <span>
              <span className="block text-sm font-semibold">Enable automatic retention</span>
              <span className="text-xs text-[var(--text-secondary)]">Equivalent to VPOS_RETENTION_ENABLED. The worker rechecks this station setting every minute.</span>
            </span>
          </label>
          <label className="flex items-start gap-3 rounded-2xl border border-[var(--border-default)] bg-[var(--surface-muted)] p-4">
            ...
            <span>
              ...
              <span className="text-xs text-[var(--text-secondary)]">Equivalent to VPOS_RETENTION_DRY_RUN. Keep enabled while validating policy values.</span>
            </span>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label className="space-y-1 text-xs font-semibold text-[var(--text-secondary)]">
            ...
            <div className="flex items-center gap-2">
              <Input type="number" min={1} value={String(Math.round(policy.cleanupIntervalMs / 60000))} onChange={(event) => setPolicy({ ...policy, cleanupIntervalMs: Math.max(1, Number(event.target.value || 1)) * 60000 })} />
              <span className="font-normal">minutes</span>
            </div>
          </label>
          <label className="space-y-1 text-xs font-semibold text-[var(--text-secondary)]">
            <span>Batch size</span>
            <Input type="number" min={1} max={5000} value={String(policy.batchSize)} onChange={(event) => setPolicy({ ...policy, batchSize: Number(event.target.value || 1) })} />
          </label>
          <label className="space-y-1 text-xs font-semibold text-[var(--text-secondary)]">
            <span>Maximum batches per run</span>
            <Input type="number" min={1} max={100} value={String(policy.maxBatches)} onChange={(event) => setPolicy({ ...policy, maxBatches: Number(event.target.value || 1) })} />
          </label>
        </div>
          ...
          <div key={group.title} className="space-y-3 rounded-2xl border border-[var(--border-default)] p-4">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              {group.fields.map((field) => (
                <DayInput key={field.key} label={field.label} value={policy[field.key]} onChange={(value) => setDay(field.key, value)} />
              ))}
            </div>
          </div>
        ))}
        ...
        <div className="space-y-3 rounded-2xl border border-[var(--border-default)] p-4">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            <label className="space-y-1 text-xs font-semibold text-[var(--text-secondary)]">
              ...
              <Input type="number" min={1} max={1000} value={String(policy.configVersionLimit)} onChange={(event) => setPolicy({ ...policy, configVersionLimit: Number(event.target.value || 1) })} />
            </label>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <label className="flex items-start gap-3 rounded-xl bg-[var(--surface-muted)] p-3">
              <Checkbox checked={policy.forecourtPayloadCompactionEnabled} onChange={() => setPolicy({ ...policy, forecourtPayloadCompactionEnabled: !policy.forecourtPayloadCompactionEnabled })} />
              <span className="text-sm">Enable forecourt payload compaction</span>
            </label>
            <label className="flex items-start gap-3 rounded-xl bg-[var(--surface-muted)] p-3">
              <Checkbox checked={policy.forecourtPayloadDryRun} onChange={() => setPolicy({ ...policy, forecourtPayloadDryRun: !policy.forecourtPayloadDryRun })} />
              <span className="text-sm">Forecourt payload compaction dry-run</span>
            </label>
          </div>
        </div>
      </CardContent>
