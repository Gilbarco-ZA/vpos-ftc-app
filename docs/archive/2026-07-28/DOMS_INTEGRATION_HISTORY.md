...
## Forecourt commissioning and recovery workspace consolidation

_Source: `FORECOURT_COMMISSIONING_PASS.md` (consolidated and removed 2026-07-21)_

This pass converted `/admin/forecourt` into a technician-oriented workspace and improved tank live-data presentation.

Delivered:

- summarized delivery and tank-gauge state with raw responses retained in collapsed technical details;
- a station-scoped commissioning checklist stored under `forecourt.domsCommissioningChecklist.v1`;
- audit and forecourt event records for checklist changes;
- supervised replay restoration with explicit `RESTORE` confirmation and no controller-buffer clearing;
- CSV-based FTC mapping template, dry-run validation, and controlled apply workflow;
- forecourt workspace grouping for commissioning, configuration, operations, maintenance, and diagnostics.

The checklist and restoration workflow do not send DOMS/PSS commands. Replay restoration creates an FTC-side `OPEN` transaction only after explicit administrator confirmation.

---

## DOMS data lifecycle pass - normalized ownership and recovery-safe payload compaction

Date: 2026-07-21

This pass adds explicit lifecycle evidence around JPL transaction payloads. Normalized transactions, raw forecourt rows, transaction checkpoints, and supervised replay rows are linked to the same transaction identity. Large JSON bodies are eligible for compaction only after normalization, durable line persistence, reconciliation, successful controller clear, no active or errored recovery claim, and a configurable grace period.

Compaction is disabled and dry-run by default, uses bounded station-scoped batches, and preserves identifiers, hashes, outcome metadata, timestamps, and clear reasons. New controller payloads reset prior clear markers. Support bundles and admin diagnostics report lifecycle counts without requiring the raw body to remain present.

The pass does not bound `forecourt_events`, compact tank-gauge payloads, remove unattended duplicates, or delete terminal replay/checkpoint rows; those remain Phase 4 follow-up work.
