...
const sql = {
  ...
  upsertByKey: `
    INSERT INTO forecourt_jpl_supervised_replay (
      station_id, fp_id, trans_seq_no, replay_stage, lock_id, read_payload_json, clear_fields_json,
      captured_at, cleared_at, last_error, updated_at
    )
    VALUES (
      $1, $2, $3, COALESCE($4, 'discovered'), $5, $6::jsonb, $7::jsonb, $8::timestamptz, $9::timestamptz, $10, NOW()
    )
...
export const forecourtJplReplayRepo = {
  async getByKey(args: {
    ...
    const res = await query(sql.selectByKey, [
      args.stationId,
      args.fpId,
      args.transSeqNo,
    ])
    const row = res.rows?.[0]
    return row ? (row as SupervisedReplayRow) : null
  },
  async upsert(args: {
    stationId: string
    fpId: number
    ...
    await query(sql.upsertByKey, [
      ...
      args.readPayloadJson != null
        ...
        : null,
      args.clearFieldsJson != null
        ? JSON.stringify(args.clearFieldsJson)
        : null,
