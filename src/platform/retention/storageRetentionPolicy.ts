...
export function getStorageRetentionPolicy(
  ...
  return {
    ...
    printTestDoneDays: retentionDays(
      source,
      'VPOS_RETENTION_PRINT_TEST_DONE_DAYS',
      DEFAULTS.printTestDoneDays,
    ),
...
const intValue = (
  ...
  return Number.isFinite(n)
    ? clampInteger(n, min, max)
    : clampInteger(fallback, min, max)
}
const dayValue = (value: unknown, fallback: number) =>
  intValue(value, fallback, 0, 3650)

export function normalizeStorageRetentionPolicy(
  input: StorageRetentionPolicyInput | null | undefined,
  fallback: StorageRetentionPolicy = getStorageRetentionPolicy(),
): StorageRetentionPolicy {
  const value = input ?? {}
  return {
    enabled: boolValue(value.enabled, fallback.enabled),
    dryRun: boolValue(value.dryRun, fallback.dryRun),
    cleanupIntervalMs: intValue(
      value.cleanupIntervalMs,
      fallback.cleanupIntervalMs,
      60_000,
      7 * 24 * 60 * 60 * 1000,
    ),
    ...
    printTestDoneDays: dayValue(
      value.printTestDoneDays,
      fallback.printTestDoneDays,
    ),
    printFailedDays: dayValue(value.printFailedDays, fallback.printFailedDays),
    transactionQueueDoneDays: dayValue(
      value.transactionQueueDoneDays,
      fallback.transactionQueueDoneDays,
    ),
    transactionQueueFailedDays: dayValue(
      value.transactionQueueFailedDays,
      fallback.transactionQueueFailedDays,
    ),
    reportQueueDoneDays: dayValue(
      value.reportQueueDoneDays,
      fallback.reportQueueDoneDays,
    ),
    reportQueueFailedDays: dayValue(
      value.reportQueueFailedDays,
      fallback.reportQueueFailedDays,
    ),
    fiscalInboxProcessedDays: dayValue(
      value.fiscalInboxProcessedDays,
      fallback.fiscalInboxProcessedDays,
    ),
    fiscalInboxResolvedDeadDays: dayValue(
      value.fiscalInboxResolvedDeadDays,
      fallback.fiscalInboxResolvedDeadDays,
    ),
    ...
    forecourtRoutineEventDays: dayValue(
      value.forecourtRoutineEventDays,
      fallback.forecourtRoutineEventDays,
    ),
    forecourtErrorEventDays: dayValue(
      value.forecourtErrorEventDays,
      fallback.forecourtErrorEventDays,
    ),
    forecourtMaintenanceSecurityEventDays: dayValue(
      value.forecourtMaintenanceSecurityEventDays,
      fallback.forecourtMaintenanceSecurityEventDays,
    ),
    forecourtFieldEvidenceEventDays: dayValue(
      value.forecourtFieldEvidenceEventDays,
      fallback.forecourtFieldEvidenceEventDays,
    ),
    jplCheckpointClearedDays: dayValue(
      value.jplCheckpointClearedDays,
      fallback.jplCheckpointClearedDays,
    ),
    jplSupervisedReplayClearedDays: dayValue(
      value.jplSupervisedReplayClearedDays,
      fallback.jplSupervisedReplayClearedDays,
    ),
    configVersionLimit: intValue(
      value.configVersionLimit,
      fallback.configVersionLimit,
      1,
      1000,
    ),
    configVersionMinAgeDays: dayValue(
      value.configVersionMinAgeDays,
      fallback.configVersionMinAgeDays,
    ),
    pssParsedCompatibilityDays: dayValue(
      value.pssParsedCompatibilityDays,
      fallback.pssParsedCompatibilityDays,
    ),
    forecourtPayloadCompactionEnabled: boolValue(
      value.forecourtPayloadCompactionEnabled,
      fallback.forecourtPayloadCompactionEnabled,
    ),
    forecourtPayloadDryRun: boolValue(
      value.forecourtPayloadDryRun,
      fallback.forecourtPayloadDryRun,
    ),
    forecourtPayloadGraceDays: dayValue(
      value.forecourtPayloadGraceDays,
      fallback.forecourtPayloadGraceDays,
    ),
  }
}
...
export async function getStationStorageRetentionPolicy(stationId: string) {
  ...
  const stored = await kvGet<StorageRetentionPolicyInput>(
    stationId,
    STORAGE_RETENTION_SETTINGS_KEY,
  ).catch(() => null)
  return normalizeStorageRetentionPolicy(stored, fallback)
}

export async function saveStationStorageRetentionPolicy(
  stationId: string,
  input: StorageRetentionPolicyInput,
) {
  const policy = normalizeStorageRetentionPolicy(
    input,
