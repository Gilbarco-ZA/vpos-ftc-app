...
test('Tanzania pre-fiscal receipt reserves verification prefix plus global counter', () => {
  ...
  assert.match(receipt, /dateParts\(assignment\.invoice_date, assignment\.timezone\)/)
  assert.doesNotMatch(receipt, /toISOString\(\)/)
})
...
test('before-transaction TIN allocation is available to operational roles with a DOMS pump board', () => {
  ...
  assert.match(page, /stationCountry=\{user\.station\?\.country \?\? null\}/)
  ...
  assert.match(board, /Capture customer/)
  assert.match(board, /<CustomerDrawer/)
  assert.match(board, /onSaved=\{handleCustomerSaved\}/)
